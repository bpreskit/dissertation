...
\begin{lemma}
  If $||L - \hL||_2 < \frac{\tau}{1 + \sqrt{n}}$, then $\hz$ meets the conditions of \cref{lem:necsuff_ang_sync_dual} and $\hZ = \hz \hz^*$ is the unique optimizer of \eqref{eq:ang_sync_sdp} and $\mfr(\hZ)$ is the unique optimizer of \eqref{eq:ang_sync_real}.
...
\begin{proof}[Proof of \cref{lem:unique_ang_sync}]
  ...
  To this end, we introduce $\hL = D - W \circ \hz \hz^*$, which has that $\Nul(\hL) = \Span(\hz)$ and $P^* \hL P \succ 0$ when $G$ is connected (see, e.g., lemma 1.7 of \cite{chungspectral}), so that \[\hL - \diag \Re \hz \hz^* \hL = \hL.\]  By Weyl's inequalities (Theorem 4.3.1 in \cite{horn2012matrix}), the smallest eigenvalue $\lambda_1(P^* Y P)$ of $P^* Y P$ satisfies \[\lambda_1(P^* Y P) \ge \lambda_1(P^* \hL P) - ||P^* (Y - \hL) P||_2 = \lambda_2(\hL) - ||Y - \hL||_2,\] so it suffices to have $\lambda_2(\hL) = \tau > ||Y - \hL||_2$.  The lemma follows by observing that
  \begin{align*}
    ||Y - \hL||_2 &\le ||L - \hL||_2 + ||\diag\Re(\hz \hz^* L)||_2 \\
    &\le ||L - \hL||_2 + ||L \hz||_\infty \\
    &\le ||L - \hL||_2 + ||L \hz||_2 \\
    &=   ||L - \hL||_2 + ||(L - \hL) \hz||_2 \\
    &\le ||L - \hL||_2 + \sqrt{n} ||L - \hL||_2, \\
  \end{align*}  
\end{proof}

\begin{proof}[Proof of \cref{lem:error_ang_sync}]
  We begin by assuming that $\ux^* \hz = \hz^* \ux = |\ux^* \hz|$, which is accomplished by taking appropriate representatives of $\ux$ and $\hz$ in $(\Sbb^1)^n / \Sbb^1$.  This gives that \[\min_{\theta \in [0, 2 \pi)} ||\ux - \ee^{\ii \theta} \hz||_2 = ||\ux - \hz||_2.\]

  Writing $\Delta L = L - \uL$, we have, by optimality of $\hz$, that $\hz^* L \hz \le \ux^* L \ux$.  Since $\ux \in \Nul(\uL)$, this gives \[\hz^* \uL \hz + \hz^* \Delta L \hz = \hz^* L \hz \le \ux^* L \ux = \ux^* \uL \ux + \ux^* \Delta L \ux = \ux^* \Delta L \ux,\] which yields \begin{equation} \begin{aligned} \hz^* \uL \hz &\le \ux^* \Delta L \ux - \hz^* \Delta L \hz \\ &= (\ux - \hz)^* \Delta L (\ux + \hz) \\ &\le ||\ux - \hz||_2 ||\Delta L||_2 \sqrt{2 n} \end{aligned} \label{eq:ang_sync_error_bound}\end{equation}  We then lower-bound the left-hand side of \eqref{eq:ang_sync_error_bound} by setting \[y = \mathrm{Proj}_{\ux^\perp} \hz = \hz - \frac{1}{n} \ux \ux^* \hz,\] so that \[\hz^* \uL \hz = y^* \uL y.\]  At this point, we remark that $||\ux \ux^* - \hz \hz^* ||_F^2 = 2n^2 - 2 |\ux^* \hz|^2$ and $||\ux - \hz||_2^2 = 2 n - 2|\ux^* \hz|$, such that \begin{align*} n ||\ux - \hz||_2^2 &\le ||\ux \ux^* - \hz \hz^* ||_F^2 = 2 (n - |\ux^* \hz|) (n + |\ux^* \hz|) \\ &= ||\ux - \hz||_2^2(n + |\ux^* \hz|) \le 2 n ||\ux - \hz||_2^2. \end{align*}  Therefore, we may observe that \[||y||_2^2 = ||\hz||_2^2 - \frac{1}{n^2} |\ux^* \hz|^2 ||\ux||_2^2 = n - \frac{1}{n} |\ux^* \hz|^2 = \dfrac{||\ux \ux^* - \hz \hz^2||_F^2}{2 n},\] giving $\frac{1}{2} ||\ux - \hz||_2^2 \le ||y||_2^2 \le ||\ux - \hz||_2^2$.  In this way, since $y$ is orthogonal to the null space of $\uL$, we have \[\hz^* \uL \hz = y^* \uL y \ge \lambda_2(\uL) \lVert y \rVert_2^2 \ge \frac{\lambda_2(\uL)}{2} ||\ux - \hz||_2^2.\]  Combining this with \eqref{eq:ang_sync_error_bound} completes the proof.
\end{proof}
